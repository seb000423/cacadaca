...
(function () {
  ...
  window.PT_DICT_APP = {
    ...
    /* 셀 요약 줄은 숫자마다 <b> 로 끊겨 있어 토막이 각각 텍스트 노드다.
       「셀 A-02 · 로봇 3대 · 레일 2축 + 갠트리 1축」 순서로 읽힌다. */
    '셀': 'Cell',
    '· 로봇': '· robots',
    '대 · 레일': '· rail',
    '축 + 갠트리': 'axes + gantry',
  ...
  /* ── 언어 전환 상자 ────────────────────────────────────────
     이 세 화면에는 헤더가 각자 따로 있고, 콘솔·라이브러리는 번들이라
     마크업을 직접 고치기 어렵다. 그래서 화면이 그려진 뒤에 끼워 넣는다.
     i18n.js 는 '.lang a[data-lang]' 을 보고 동작하므로 모양만 맞추면 된다.

     화면마다 붙일 자리가 다르다. 위에서부터 처음 맞는 것 하나만 쓴다.
     선택자가 실제로 맞는지는 scripts/i18n-app-check.py --anchors 가 본다. */
  var ANCHORS = [
    '.bar',            /* ② 모니터 — 상단 스테이터스 바 */
    'main nav',        /* ④ 라이브러리 — ①②③ 링크가 있는 곳 */
    'main > header'    /* ① 콘솔 — 3D 뷰 위에 뜨는 머리말 */
  ];
