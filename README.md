...
새 화면을 공개하려면 `server/server.js` 의 `PUBLIC_PAGES` 에 넣어야 한다.
...
middleware.js        Edge HTML 게이팅 (HMAC 쿠키 검증)
server/              routes.js · db.js · auth.js · server.js (로컬 서버)
console-src/         ① 화면 편집 원본 + repack.py
library-src/         ③④ 화면 편집 원본 + repack.py
save-src/            저장 화면 편집 원본 + repack.py
scripts/             폰트 서브셋 · 이미지 보정 · 검수 · 관리자 시드
assets/models/       meshopt 압축 모델 (_orig/ 는 비교용, 배포 제외)
assets/fonts/        Pretendard 서브셋
assets/vendor/       three.js r169 + 애드온 + MeshoptDecoder
데이터셋/             sweep 궤적 CSV, KPI JSON, 생성 스크립트
```
...
검수는 로컬 서버를 띄우고 `asset-check.html` 로 한다 — 압축 모델을 원본과 나란히 놓고 본다.
...
| [UI목업_프롬프트_폴리트윈_2026-08-25.md](UI목업_프롬프트_폴리트윈_2026-08-25.md) | 원래 기획 의도와 화면별 요구사항 |
| [AI티제거_지침서_폴리트윈_2026-08-27.md](AI티제거_지침서_폴리트윈_2026-08-27.md) | 디자인 판단의 배경 |
