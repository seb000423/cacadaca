...
while [ "$i" -lt "$n" ]; do
  ...
  RAW=learning/rl/robot/results/rerun_raw; mkdir -p "$RAW"
  timeout 3600 "$PY" learning/rl/car_cells_robot.py --headless --checkpoint "$CKPT" \
      --cells "$cells" --num_envs "${#chunk[@]}" --out "$OUT" \
      --force_scale "$FS" --pad_radius "$PR" --tag "$TAG" 2>&1 | tee "$RAW/batch_$(date +%H%M%S).log" | grep -E "\[car_cells\]|Traceback|Error\]|Error:|Exception" | grep -v "omni.kit.app"
