#!/usr/bin/env python3
"""Compare curved RTX scans before and after simulated polishing."""
...
def main():
    args = parse_args()
    before = read(args.before_dir / "curved_rtx_gloss_results.csv")
    after = read(args.after_dir / "curved_rtx_gloss_results.csv")
    if len(before) != 25 or len(after) != 25:
        raise RuntimeError("Expected matching 5x5 curved scans")
    combined = []
    for before_row, after_row in zip(before, after):
        ...
        if before_cell != after_cell:
            raise RuntimeError("Before/after cell ordering differs")
    ...
    for axis, data, title, colour_map in (
        (axes[0], before_grid, "Curved before\nrelative gloss, not GU", "viridis"),
        (axes[1], after_grid, "Curved after\nrelative gloss, not GU", "viridis"),
        (axes[2], improvement, "Improvement\nafter - before", "RdYlGn"),
    ):
    ...
    plot_path = args.output_dir / "curved_before_after_heatmaps.png"
    ...
    summary = {
        "mode": "curved_distributed_roughness_before_after_rtx",
        ...
        "passed": after_mean > before_mean + 0.05 and improved_count >= 20,
    }
    summary_path = args.output_dir / "curved_comparison_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print("")
    print("곡면 분산결함 폴리싱 전·후 비교")
    ...
    if not summary["passed"]:
        raise RuntimeError(f"Curved before/after validation failed; see {summary_path}")
